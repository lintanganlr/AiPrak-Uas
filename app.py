...
environmental_factors = {
    "E01": "Banyak orang merokok di sekitar",
    "E02": "Kondisi udara buruk (polusi, asap)",
    "E03": "Lingkungan berdebu",
    "E04": "Udara dingin atau lembab",
    "E05": "Kehadiran hewan peliharaan"
}

triggers = {
    "T01": "Kambuh saat stress",
    "T02": "Kambuh saat gugup",
    "T03": "Kambuh saat olahraga",
    "T04": "Kambuh saat terpapar alergen (debu, bulu hewan)",
    "T05": "Kambuh saat terkena infeksi pernapasan"
}
...
hubungan_data = {
    ...
    ("E01", "P01"): {"MB": 0.6, "MD": 0.1},
    ("T01", "P01"): {"MB": 0.5, "MD": 0.2},
    ...
    ("E02", "P01"): {"MB": 0.6, "MD": 0.2},
    ("E03", "P05"): {"MB": 0.7, "MD": 0.2},
    ("E04", "P09"): {"MB": 0.5, "MD": 0.3},
    ("E05", "P01"): {"MB": 0.6, "MD": 0.1},
    ("T02", "P01"): {"MB": 0.5, "MD": 0.3},
    ("T03", "P10"): {"MB": 0.7, "MD": 0.2},
    ("T04", "P01"): {"MB": 0.8, "MD": 0.2},
    ("T05", "P09"): {"MB": 0.6, "MD": 0.3},
    ("E01", "P09"): {"MB": 0.6, "MD": 0.3},
    ("E02", "P09"): {"MB": 0.5, "MD": 0.3},
    ("E03", "P09"): {"MB": 0.7, "MD": 0.3},
    ("E04", "P09"): {"MB": 0.4, "MD": 0.2},
    ("E05", "P09"): {"MB": 0.6, "MD": 0.3},
}
...
def index():
    try:
        ...
        return render_template('main.html', bg_url=bg_url, depart_url=depart_url, eyecare_url=eyecare_url, aboutUs_url=aboutUs_url, about_url=about_url,
                               ...
                               blog_url=blog_url, ctabg_url=ctabg_url, gejala_data=gejala_data, environmental_factors=environmental_factors, triggers=triggers)
...
def diagnose():
    try:
        ...
        gejala_input = request.form.getlist('inputGejala')
        environmental_input = request.form.getlist('inputEnvironmental')
        trigger_input = request.form.getlist('inputTrigger')

        all_input = gejala_input + environmental_input + trigger_input
        ...
        for penyakit, CF in sorted_CF_dict.items():
            if CF > 0:
                hasil_diagnosis.append({
                    ...
                    "cf": f"{CF * 100:.2f}%",
                    "gejala": [gejala_data.get(g, environmental_factors.get(g, triggers.get(g))) for g in penyakit_terkait_dict[penyakit] if g in all_input]
                })
        ...
        return render_template('result.html', name=name, usia=usia, hasil_diagnosis=hasil_diagnosis, gejala_input=gejala_input, environmental_input=environmental_input, trigger_input=trigger_input, gejala_data=gejala_data, environmental_factors=environmental_factors, triggers=triggers)
    except Exception as e:
        return str(e)
